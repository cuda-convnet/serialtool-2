...
void MyComTool::comStrat_Stop_Con()
{
    ...
    if(com_status == false)
    {
        ...
        if(com_Num == "ttyS0")
            fd_com = open(ttyS0, O_RDWR);
        else if(com_Num == "ttyS1")
            fd_com = open(ttyS1, O_RDWR);
        else if(com_Num == "ttyS2")
            fd_com = open(ttyS2, O_RDWR);
        else if(com_Num == "ttyS3")
            fd_com = open(ttyS3, O_RDWR);
        else if(com_Num == "ttyUSB0")
            fd_com = open(ttyUSB0, O_RDWR);
        else if(com_Num == "ttyUSB1")
            fd_com = open(ttyUSB1, O_RDWR);
        else
            fd_com = open(com_Num.toAscii(), O_RDWR);
        if(fd_com != -1){
            path = ":/on.jpg";
            serialstatusLabel->setPixmap(path);
            openCloseSerialPushButton->setText(tr("关闭串口"));
            com_all_status.replace(com_Num.length ()+2,  5, "Used");
            com_status = true;
            ...
            if(com_Num == "ttyS0") {
                fds = open(ttyS0, O_WRONLY);
                fdr = open(ttyS0, O_RDONLY);
            } else 	if(com_Num == "ttyS1") {
                fds = open(ttyS1, O_WRONLY);
                fdr = open(ttyS1, O_RDONLY);
            } else if(com_Num == "ttyS2") {
                fds = open(ttyS2, O_WRONLY);
                fdr = open(ttyS2, O_RDONLY);
            } else if(com_Num == "ttyS3") {
                fds = open(ttyS3, O_WRONLY);
                fdr = open(ttyS3, O_RDONLY);
            }  else if(com_Num == "ttyUSB0") {
                fds = open(ttyUSB0, O_WRONLY);
                fdr = open(ttyUSB0, O_RDONLY);
            }  else if(com_Num == "ttyUSB1") {
                fds = open(ttyUSB1, O_WRONLY);
                fdr = open(ttyUSB1, O_RDONLY);
            } else  {
                fds = open(com_Num.toAscii(), O_WRONLY);
                fdr = open(com_Num.toAscii(), O_RDONLY);
            }
            myComThread->start();
        }else{
            path = ":/error.jpg";
...
void MyComTool::manualCom_Sent()
{
    ...
    if(!sent_hex_model){
        ...
        QStringList strlist =user_input.split
                (QRegExp("[\n\r\t ]"),QString::SkipEmptyParts);
        int i;char c;
        if(strlist.length ()>= 10240){
            char_count = 0;
        ...
        for(i=0;i<strlist.length ();i++){
            bool isok;
            c= strlist.at (i).toUInt (&isok,16);
            if(isok){
                hex_to_sent[char_count] = c;
                char_count++;
            }
        }
